...
exports.InvalidPhoneNumberError = exports.OperatorNotFoundError = exports.UnsupportedTypeError = exports.InvalidCharacterError = exports.InvalidLengthError = exports.InvalidFormatError = exports.PhoneNumberError = void 0;
...
class InvalidLengthError extends PhoneNumberError {
    constructor(message, phoneNumber, actualLength) {
        super(message, phoneNumber);
 ...
 */
class InvalidCharacterError extends PhoneNumberError {
    constructor(message, phoneNumber, invalidCharacters) {
        super(message, phoneNumber);
 ...
 */
class UnsupportedTypeError extends PhoneNumberError {
    constructor(message, phoneNumber, phoneType) {
        super(message, phoneNumber);
 ...
 */
class OperatorNotFoundError extends PhoneNumberError {
    constructor(message, phoneNumber, operator) {
        super(message, phoneNumber);
        this.operator = operator;
        this.name = 'OperatorNotFoundError';
    }
}
exports.OperatorNotFoundError = OperatorNotFoundError;
/**
 * Legacy error class for backward compatibility
 * @deprecated Use specific error types instead
 */
class InvalidPhoneNumberError extends PhoneNumberError {
    constructor(message, phoneNumber) {
        super(message, phoneNumber);
...
function formatPhone(phone, type = "international") {
    ...
    if (!/^[0-9+\-()\s]+$/.test(phone)) {
        ...
        throw new InvalidCharacterError(`Phone number contains invalid characters: [${invalidChars.join(', ')}]. Only digits, +, -, (, ), and spaces are allowed.`, phone, invalidChars);
    }
    const formatted_phone = phone.replace(/[^+\d]/g, "");
    const phone_length = formatted_phone.length;
    // Length validation
    if (phone_length < 9 || phone_length > 13) {
        throw new InvalidLengthError(`Phone number length must be between 9 and 13 digits, got ${phone_length}`, phone, phone_length);
    }
    ...
    if (phone_length === 13 && formatted_phone.startsWith("+251")) {
        ...
        throw new InvalidFormatError(`Invalid phone number format: ${phone}. Expected format: 09XXXXXXXX, 07XXXXXXXX, 9XXXXXXXX, 7XXXXXXXX, +251XXXXXXXXX, or 251XXXXXXXXX`, phone);
    }
}
/**
...
function formatLocal(phone) {
    ...
    throw new InvalidFormatError(`Phone number must start with +251, got: ${phone}`, phone);
}
/**
